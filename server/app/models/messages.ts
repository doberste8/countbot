...
function messages() {
    ...
    this.get = function(user_id, startDate, endDate, text, res) {
        db.acquire(function(err, con) {
            ...
            con.query('SELECT * FROM messages WHERE user_id IN (SELECT id FROM users WHERE name LIKE ?) AND created_at BETWEEN ? AND ? AND text like ?', [user_id, startDate, endDate, text],
                function(err, result) {
                    con.release();
                    if (err) {
                        // throw err;
    ...
    this.getCount = function(userName, startDate, endDate, queryText, res) {
        db.acquire(function(err, con) {
            if (err) throw err; // You *MUST* handle err and not continue execution if
            // there is an error. this is a standard part of Node.js
            con.query('CALL GetCounts(?,?,?,?)', [userName, startDate, endDate, queryText],
                function(err, result) {
                    con.release();
                    if (err) {
                        // throw err;
    ...
    this.create = function(data, res) {
        db.acquire(function(err, con) {
            if (err) throw err; // You *MUST* handle err and not continue execution if
            // there is an error. this is a standard part of Node.js
            con.query('INSERT INTO messages (id, created_at, user_id, text) VALUES ? ON DUPLICATE KEY UPDATE id=id', [data],
                function(err, result) {
                    con.release();
                    if (err) {
                        // throw err
