...
function likes() {
    ...
    this.get = function(message_id, user_id, res) {
        db.acquire(function(err, con) {
            ...
            con.query('select * from likes where message_id like ? and user_id like ?', [message_id, user_id],
                function(err, result) {
                    con.release();
                    if (err) {
                        // throw err;
    ...
    this.getMatrix = function(startDate, endDate, res) {
        db.acquire(function(err, con) {
            if (err) throw err; // You *MUST* handle err and not continue execution if
            // there is an error. this is a standard part of Node.js
            con.query('call LikeMatrix(?,?)', [startDate, endDate],
                function(err, result) {
                    con.release();
                    if(err) {
                        // throw err;
    ...
    this.create = function(data, res) {
        db.acquire(function(err, con) {
            if (err) throw err; // You *MUST* handle err and not continue execution if
            // there is an error. this is a standard part of Node.js
            con.query('INSERT INTO likes (message_id, user_id) VALUES ? ON DUPLICATE KEY UPDATE message_id=message_id', [data],
                function(err, result) {
                    con.release();
                    if (err) {
                        // throw err
